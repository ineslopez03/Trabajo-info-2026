...
void MenuPrincipal::procesarEntrada(sf::RenderWindow& ventana, MotorArchon* motor) {//metodo para procesar las entradas del usuario
    ...
    auto actualizar = [&](std::vector<Boton>& lista) {//funcion lamda que actualiza el color de los botones segun el raton
    ...
    switch (EstadoInterno) {//se usa un switch para actualizar el color de los botones segun el estado del menú(la funcion lamda)
        ...
        if (evento->is<sf::Event::Closed>()) {//si hay un evento de cierre de ventana, se cierra la ventana
            ...
            if (click->button == sf::Mouse::Button::Left) {//y es el click izquierdo
                if (EstadoInterno == OpcionesMenu::PRINCIPAL) {//transitamos al menu que toque segun el boton pulsado
                    ...
                    if (BotonesMenuPrincipal[1].botonContieneRaton(PosRaton)) motor->encolarCambioEstado(EstadoJuego::RANKING);
                    if (BotonesMenuPrincipal[2].botonContieneRaton(PosRaton)) EstadoInterno = OpcionesMenu::MANUAL_GRAFICO;
                    if (BotonesMenuPrincipal[3].botonContieneRaton(PosRaton)) motor->encolarCambioEstado(EstadoJuego::FIN);
                ...
                else if (EstadoInterno == OpcionesMenu::SELECCION_SKIN) {//Aquí seleccionamos la skin y delegamos la inicialización del juego al motor
                    if (BotonesSeleccionSkin[0].botonContieneRaton(PosRaton)) { motor->encolarCambioEstado(EstadoJuego::TABLERO, nullptr, nullptr, "ARCHON"); }
                    else if (BotonesSeleccionSkin[1].botonContieneRaton(PosRaton)) { motor->encolarCambioEstado(EstadoJuego::TABLERO, nullptr, nullptr, "HARRY_POTTER"); }
                    else if (BotonesSeleccionSkin[2].botonContieneRaton(PosRaton)) { motor->encolarCambioEstado(EstadoJuego::TABLERO, nullptr, nullptr, "STAR_WARS"); }
...
void MenuPrincipal::dibujarPantalla(sf::RenderWindow& ventana) {//metodo que usamos para pintar la ventana
    ...
    switch (EstadoInterno) {//switch para ppintar el titulo sobre el fondo segun el estado
    ...
    auto dibujarLista = [&](std::vector<Boton>& lista) { for (auto& b : lista) b.dibujar(ventana); };//funcion lamda para recorrer los botones y dibujarlos
    switch (EstadoInterno) {//switch para dibujar los botones segun el estado del menú usando la función lamda
...
void MenuPrincipal::cargarTextoManual() {// se carga el texto del manual desde un archivo de texto
    ...
    if (!archivo.is_open()) {//si el archivo no se ha podido abrir lanza error de texto
        std::cout << "Error cargando el archivo del manual\n\n";
    ...
    while (std::getline(archivo, linea)) {//vamos obteniendo linea a linea del archivo hasta el final
        ...
        yOffset += 20.0f; //se le va dando un offser vertical para que no se solapen las lineas
    }
